...
class SlangDiffDRRRenderer:
    ...
    def __init__(
        ...
        self._origin_xyz = tuple(float(v) for v in origin_xyz_mm)
    ...
    def _init_slang(self):
        ...
        # CUDA remains the upstream default. Vulkan uses the same Slang shader
        # and is useful when the installed CUDA toolkit emits PTX newer than
        # the host driver accepts.
        try:
            device_type = {
                "cuda": slangpy.DeviceType.cuda,
                "vulkan": slangpy.DeviceType.vulkan,
            }[self._cfg.device_type]
    ...
    def origin_xyz(self) -> tuple[float, float, float]:
        """Return the world position of voxel [0, 0, 0] in XYZ order."""
