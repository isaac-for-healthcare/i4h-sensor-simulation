...
class RealismConfig:
    ...
    - Output is float32 and stays in intensity units, so photon statistics apply to physical
      values and consecutive frames remain comparable. Mapping intensity to a display range
      belongs to :mod:`xray_simulator.display`; set normalize_output=True only for a one-off
      still, since a per-frame rescale makes a cine sequence flicker.
    """
    ...
    # Output normalization (per-frame min/max; off by default, see class docstring)
