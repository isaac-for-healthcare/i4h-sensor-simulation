...
docker run --rm --gpus all -v ~/ct_data:/root/ct_data -it xray_simulator bash
...
```python
from xray_simulator import VolumePreprocessor, PreprocessedVolume
...
| 4. Saving Results | Saves frames to `/tmp/xray_simulator_output/frames/` |
...
>
> - `/tmp/xray_simulator_output/` — Saved automatically by the simulator when `save_to_disk=True` is set in `OutputSettings`
> - `/tmp/xray_simulator_output/frames/` — Saved explicitly by the demo script via `cine.save_all()`
...
```python
from xray_simulator import VolumePreprocessor, xray_simulator, Pose, PreprocessedVolume
...
# Render AP view (anterior-posterior)
frame_ap = simulator.render_frame(pose=Pose.ap())
frame_ap.save("/tmp/ap_view.png")

# Render 30° LAO view
frame_lao = simulator.render_frame(pose=Pose.from_clinical_angles(30.0))
frame_lao.save("/tmp/lao_30.png")

# Render 20° RAO + 15° cranial
frame_combined = simulator.render_frame(
    pose=Pose.from_clinical_angles(primary_deg=-20.0, secondary_deg=15.0)
)
frame_combined.save("/tmp/rao_cranial.png")
```

> The view presets assume the volume is in the canonical patient frame; see
> [World Frame and Pose Convention](architecture-and-api.md#world-frame-and-pose-convention).

### Cine Sequence (Video)

```python
from xray_simulator import xray_simulator, Pose, PreprocessedVolume

volume = PreprocessedVolume.load("/tmp/fluoro_cache")
simulator = xray_simulator(volume)

# Generate a rotational angiography sweep, RAO 90 through LAO 90
poses = [
    Pose.from_clinical_angles(primary_deg=angle)
    for angle in range(-90, 91, 3)  # 61 frames, 3° per frame
]
...
frames = cine.to_numpy()  # Shape: (61, 512, 512)
```
...
def pose_generator():
    ...
    angle = 0.0
    while True:
        yield Pose.from_clinical_angles(primary_deg=angle)
        angle += 0.5  # Continuous LAO rotation, degrees per frame
...
```python
import torch
from xray_simulator.rendering.diffdrr_slang_renderer import TorchSlangDiffDRR, SlangDiffDRRConfig
from xray_simulator import PreprocessedVolume
...
### Step 2: Run the Simulator Demo (`xray_simulator_demo.py`)
...
2. Initializes `xray_simulator` with the configured detector resolution
3. Performs GPU warmup for accurate FPS measurement
4. Renders a sequence of frames in a LAO/RAO (left/right oblique) sweep
5. Saves frames to `/tmp/xray_simulator_output/frames/`
...
Output frames are saved to `/tmp/xray_simulator_output/frames/` as PNG images.
...
ls /tmp/xray_simulator_output/frames/
