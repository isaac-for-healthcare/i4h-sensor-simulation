...
This script demonstrates the complete xray_simulator workflow:
...
To clear cache and reload from source:
    rm -rf /tmp/fluoro_cache && python examples/xray_simulator_demo.py
"""
...
# Output directory
OUTPUT_DIR = Path(os.environ.get("xray_simulator_OUTPUT_DIR", str(_SCRIPT_DIR / "output"))).expanduser()

# Cache directory for preprocessed volume
CACHE_DIR = Path(os.environ.get("xray_simulator_CACHE_DIR", str(OUTPUT_DIR / "cache"))).expanduser()
...
def main():
    ...
    # Create rotation sequence: RAO 30 through LAO 30, with a small cranial/caudal wobble
    num_frames = 20
    poses = []
    for i in range(num_frames):
        primary_deg = -30 + (60 * i / (num_frames - 1))
        secondary_deg = 5 * math.sin(2 * math.pi * i / num_frames)
        poses.append(Pose.from_clinical_angles(primary_deg, secondary_deg))
    ...
    # An AP view in both modes: fluoroscopy (dense dark) and X-ray (dense bright). Calibrating
    # first fits the display window to this patient, and polarity is the only difference.
    simulator.calibrate_display()
    simulator.render_frame(pose=Pose.ap()).save(OUTPUT_DIR / "ap_fluoro.png")
    simulator.set_polarity("xray")
    simulator.render_frame(pose=Pose.ap()).save(OUTPUT_DIR / "ap_xray.png")
    simulator.set_polarity("fluoro")

    print(f"  Saved ap_fluoro.png and ap_xray.png to: {OUTPUT_DIR}")
