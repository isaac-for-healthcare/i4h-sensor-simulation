           ...
           [](raysim::RaytracingUltrasoundSimulator& self,
             ...
             auto parse_output_size = [](py::object size) -> uint2 {
                 if (py::isinstance<py::array>(size)) {
                     ...
                     if (array.dtype().kind() == 'i' || array.dtype().kind() == 'u' || array.dtype().kind() == 'f') {
                         try {
                           return make_uint2(py::cast<uint32_t>(array[py::int_(0)]),
                                             py::cast<uint32_t>(array[py::int_(1)]));
                         } catch (const py::cast_error&) {
                             throw std::runtime_error("output_size numpy array elements cannot be converted to uint32_t");
                         }
                     } else {
             ...
             // Call the C++ method
             auto result_mem = self.generate_sector_mask(probe, t_far, output_size,
