...
- **Resolution Modeling**: A single depth-invariant PSF approximates the key resolution limits of real ultrasound:
  - Axial resolution: Limited by pulse length, related to transducer frequency
  - Lateral resolution: Determined by beam width, related to aperture size
...
1. **Beamforming Transformation**: Beamforming effectively transforms near-field waves into far-field-like behavior, though the approximation degrades close to the transducer face
...
This ray-based approach trades physical completeness for the computational efficiency needed for real-time simulation.
...
### 1.4 What the Ray Model Does and Does Not Capture

The simulator follows the straight-ray approach of Bürger et al. (2013) [[1]](#5-references); wave phenomena are not simulated. This trades physical completeness for speed:

| Phenomenon | Modeled? | Consequence |
| ---------- | -------- | ----------- |
| Diffraction / beam formation | No | Resolution comes from a depth-invariant Gaussian-cosine PSF — no focal zone, no side or grating lobes. |
| Interference | No | Speckle is sampled from a precomputed random 3D texture rather than emerging from coherent scatterer summation. Its appearance is plausible, not physical. |
| Phase / RF fidelity | No | The simulation is amplitude-only. Scanline data cannot be used for beamforming, Doppler, or ultrasound signal-processing research. |
| Sound-speed time of flight | No | Echo depth follows geometric path length, not acoustic time of flight. Sound speed steers rays at interfaces but does not shift echoes in range — no aberration or depth misregistration. |
| Near-field effects | Approximate | The ray approximation is least accurate close to the transducer face. |

**What this means in practice:** the simulator produces realistic-looking B-mode images at real-time rates — sufficient for sim-to-real robotics perception, RL/ML data generation, and probe-in-the-loop applications. It is not a substitute for wave solvers (e.g., k-Wave, Field II) in quantitative acoustics, and experts in ultrasound signal processing will notice the missing wave physics.

Practical scene constraints also apply:

- Input geometry must be **labeled surface meshes** (OBJ/STL) or primitive spheres; raw CT/MRI volumes must first be segmented and meshed (see [Section 4.2](#42-generating-simulation-inputs-from-ct-and-mri)).
- Mesh and sphere objects cannot currently be combined in the same world.
- The world must contain at least one object; empty worlds fail to build the ray-tracing acceleration structure.
...
- **Python Interface**: Bindings for ease of use [`ultrasound_simulator/cuda/__init__.py`]
...
This design allows the ray tracer to model complex acoustic interactions between multiple tissue types in a physically plausible way, while maintaining high performance through GPU acceleration.
...
3. **Plausible, Not Physical**: Speckle is sampled from a precomputed texture, not synthesized from coherent scatterer summation
...
[1] B. Bürger, S. Bettinghausen, M. Rädle, and J. Hesser (2013) - "Real-Time GPU-Based Ultrasound Simulation Using Deformable Mesh Models," *IEEE Transactions on Medical Imaging*, vol. 32, no. 3, pp. 609–618. [DOI: 10.1109/TMI.2012.2234474](https://doi.org/10.1109/TMI.2012.2234474) / [PubMed 23268382](https://pubmed.ncbi.nlm.nih.gov/23268382/) — **the primary reference for this simulator's ray-tracing approach**
