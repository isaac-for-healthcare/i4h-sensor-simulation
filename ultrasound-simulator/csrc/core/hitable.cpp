...
#include "ultrasound_simulator/cuda/optix_trace.hpp"

namespace ultrasound_simulator {
...
// Assimp's OBJ material parser reports illumination models it does not implement, and the organ
// meshes in the asset catalog declare `illum 3`. It ignores the value and imports the geometry
// anyway, and this loader reads only positions, faces and normals, so the message says nothing
// about whether the mesh loaded. A genuine import failure surfaces as ReadFile() returning null,
// which throws below. Logging this one at the stream's own level put an `[error]` line in front of
// every mesh the simulator loads.
static bool is_ignorable_material_diagnostic(const std::string& message) {
  return message.find("unexpected illumination model") != std::string::npos;
}
...
class AssimpLogStream : public Assimp::LogStream {
 public:
  void write(const char* message) {
    ...
    const auto level = is_ignorable_material_diagnostic(m) ? spdlog::level::debug : LEVEL;
